...
func TestEdits(t *testing.T) {
	...
	for i := 0; i < len(commitHashes)-1; i++ {
		t.Run(commitHashes[i], func(t *testing.T) {
			// ## Current commit's contents
			currentHash := commitHashes[i]
			currentContents, err := FileContentsInCommit(repo, currentHash, filePath)
			if err != nil {
				t.Fatal(err)
			}
			...
			nextContents, err := FileContentsInCommit(repo, nextHash, filePath)
			if err != nil {
				t.Fatal(err)
			}
			...
			for _, e := range edits {
				updatedContents, err = e.Apply(updatedContents)
				if err != nil {
					t.Fatal(err)
				}
			}
...
func TestDebug(t *testing.T) {
	...
	for i := 0; i < len(commitHashes)-1; i++ {
		t.Run(commitHashes[i], func(t *testing.T) {
			...
			// ## Compare the updated (current) contents and the next
			updatedBytes, err := os.ReadFile(currentFile)
			if err != nil {
				t.Fatal(err)
			}
			diff := cmp.Diff(string(updatedBytes), nextContents)
			if diff != "" {
				t.Fatalf("%s vs. %s has difference:\n%s", currentHash, nextHash, diff)
			}
		})
